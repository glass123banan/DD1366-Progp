...
class Parser:
    ...
    def consume(self, expected_type):
        ...
        if token.type == TokenType.INVALID:
            raise SyntaxError(f"Syntaxfel på rad {token.row}")
        elif token.type == expected_type:
            self.last_row = token.row
            self.current_token = self.lexer.peek_token()
            return token
        else:
            error_row = self.last_row if token.type == TokenType.EOF else token.row
    ...
    def rep(self):
        ...
        if self.current_token.type == TokenType.QUOTE:
            ...
            valid_instruction_starts = {TokenType.FORW, TokenType.BACK, TokenType.LEFT,
                                   TokenType.RIGHT, TokenType.DOWN, TokenType.UP,
                                   TokenType.COLOR, TokenType.REP}
            if self.current_token.type not in valid_instruction_starts:
                raise SyntaxError(f"Syntaxfel på rad {self.current_token.row}")
            expr_node = self.expr()
            self.consume(TokenType.QUOTE)
