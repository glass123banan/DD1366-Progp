...
class Token:
    def __init__(self, type, data=None, row=0):
        self.type = type # NUM, FORW, etc...
        self.data = data # optional data: e.g. int for NUM token
        self.row = row # Line nr for syntax errors

# Lexern - breaks down input to sequence of tokens
class Lexer:
    def __init__(self, text):
        self.text = text.upper() # convert to uppercase (case-insensitive)
        self.tokens = [] 
        self.current_token = 0
        self.tokenize() # call the tokenize func defined down below
    ...
    def tokenize(self):
        ...
        pattern = r"""(?x)
            ...
            | (REP)(\n|\040|\t|%.*\n)       # REP
            | (")                           # quote
            | ([1-9][0-9]*)(\n|\040|\t|\.|%.*\n)  # number followed by newline, space, tab, PERIOD, or comment
            | (\n)                          # newline
            | (\040|\t)+                    # whitespace
        ...
        for m in re.finditer(pattern, self.text):
            ...
            if m.group(1):  # comment
                current_row += 1
            elif m.group(2):  # PERIOD
                self.tokens.append(Token(TokenType.PERIOD, row=current_row))
            elif m.group(3):  # FORW
...
class Parser:
    ...
    def instruction(self):
        ...
        if t.type in {TokenType.FORW, TokenType.BACK}:
            ...
            try:
                self.expect_token(TokenType.PERIOD)
